...
Menu N3DSMenu = {
    "New 3DS menwu",
    {
        { "Enwable L2 cache", METHOD, .method = &N3DSMenu_EnableDisableL2Cache },
...
void N3DSMenu_UpdateStatus(void)
{
    ...
    N3DSMenu.items[0].title = L2CacheEnabled ? "Diswable L2 cache" : "Enwable L2 cache";
    sprintf(clkRateBuf, "Set cock wate to %luMHz", clkRate != 268 ? 268 : (u32)higherClkRate);
}
